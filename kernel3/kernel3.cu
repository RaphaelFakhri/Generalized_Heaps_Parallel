...
__global__ void topDownDeletion(int* deletedItems, int* heapLocks, Partial_Buffer* partialBuffer, Heap* heap) {
    ...
    while (1) {
        ...
        // Third optimization: Early breaking in delete reheapification: 
        // if the largest element in the left child is smaller than the smallest element in the right child, 
        // then the heap property is already satisfied and break early. This optimization is not present in the paper, hence
        // I consider it an optimization, although I would group early breaking / edge cases in many methods as seen elsewhere
        // in the code to all be the third optimization, i.e. improving latency hiding through better utilization
        // of the hardware.

        if (array1[BATCH_SIZE - 1] <= array2[0]) {
            copyArray(array2, 0, heap->arr, leftChild * BATCH_SIZE, BATCH_SIZE);
            __threadfence();
            if (threadId == MASTER_THREAD) {
                releaseLockAtomic(&heapLocks[leftChild], INUSE, AVAILABLE);
            }
            __syncthreads();
            break;
        }
